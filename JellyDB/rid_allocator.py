...
class RIDAllocator:
    ...
    """
    def make_page_range(self, filename: str, col_count: int) -> list:
        pages = []
        for _ in range(Config.NUMBER_OF_BASE_PAGES_IN_PAGE_RANGE):
            pages.append(self.make_base_page(filename, col_count))
        pages.append(self.make_tail_page(filename, col_count))
    ...
    """
    def make_base_page(self, filename: str, col_count: int) -> LogicalPage:
        ...
        return LogicalPage(filename, col_count, base, bound, self.bufferpool)
    
    """
    # Allocates a tail page with its own unique RID range
    :returns:   # tuple, (lowest RID allocated, highest RID allocated)
    """
    def make_tail_page(self, filename: str, col_count: int) -> LogicalPage:
